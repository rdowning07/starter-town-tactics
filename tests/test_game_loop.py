...
from game.game_loop import (
    demo_game_loop,
    game_loop,
    handle_boss_phase_effects,
    handle_reinforcement_effects,
    handle_storm_effects,
    handle_triggered_events,
...
def test_handle_triggered_events_multiple(mock_game_state):
    ...
    with patch("game.game_loop.handle_reinforcement_effects") as mock_reinforcements, patch(
        "game.game_loop.handle_storm_effects"
    ) as mock_storm:
...
def test_game_loop_with_events(mock_game_state):
    ...
    with patch("time.sleep"), patch("game.game_loop.handle_reinforcement_effects") as mock_handler:
        game_loop(mock_game_state, max_turns=1)
