        ...
        try:
            ...
            if self.status_effect and hasattr(combo_manager, "status_manager"):
                combo_manager.status_manager.add_effect(
                    unit_id, self.status_effect, duration=self.status_duration, stacks=self.status_stacks
                )
            ...
            if hasattr(combo_manager, "logger") and combo_manager.logger:
                combo_manager.logger.log_event(
                    "combo_step_error", {"unit": unit_id, "step": self.name, "error": str(e)}
                )
            return False
...
class ComboManager:
    ...
    def __init__(
        self, status_manager: Optional[StatusEffectManager] = None, fx_manager: Optional[FXManager] = None, logger=None
    ):
    ...
    def _create_combo_definitions(self) -> Dict[str, List[ComboStep]]:
        ...
        return {
            "basic_attack_chain": [
                ComboStep("slash", self._slash_attack, cooldown=1, description="Basic slash attack", fx_type="damage"),
                ComboStep("thrust", self._thrust_attack, cooldown=2, description="Follow-up thrust", fx_type="damage"),
                ComboStep(
                    "finisher", self._finisher_attack, cooldown=3, description="Combo finisher", fx_type="critical"
                ),
            ],
            "magic_chain": [
                ComboStep(
                ...
                ComboStep(
                    "explosion", self._explosion_attack, cooldown=3, description="Explosive finish", fx_type="critical"
                ),
            ],
            "defensive_chain": [
                ComboStep(
                ...
                ComboStep("counter", self._counter_attack, cooldown=2, description="Counter attack", fx_type="damage"),
            ],
            "buff_chain": [
                ComboStep(
                    "haste",
    ...
    def add_combo(self, unit_id: str, combo_name: str) -> bool:
        ...
        if combo_name not in self.combo_definitions:
            if self.logger:
                self.logger.log_event("combo_unknown", {"unit": unit_id, "combo": combo_name})
            return False
        ...
        if self.logger:
            self.logger.log_event(
                "combo_added", {"unit": unit_id, "combo": combo_name, "steps": len(self.active_combos[unit_id])}
            )
    ...
    def remove_combo(self, unit_id: str) -> bool:
        ...
        if unit_id in self.active_combos:
            ...
            if self.logger:
                self.logger.log_event("combo_removed", {"unit": unit_id})
            return True
        return False
    ...
    def execute_combo(self, unit_id: str, game_state, **kwargs) -> Dict[str, Any]:
        ...
        for step in self.active_combos[unit_id]:
            ...
            try:
                ...
                if self.logger:
                    self.logger.log_event("combo_step_exception", {"unit": unit_id, "step": step.name, "error": str(e)})
    ...
    def _slash_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 8
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_slash_attack", {"unit": unit_id, "damage": damage})
        return True
    ...
    def _thrust_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 12
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_thrust_attack", {"unit": unit_id, "damage": damage})
        return True
    ...
    def _finisher_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 20
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_finisher_attack", {"unit": unit_id, "damage": damage})
        return True
    ...
    def _fireball_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 15
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_fireball_attack", {"unit": unit_id, "damage": damage})
        return True
    ...
    def _explosion_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 25
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_explosion_attack", {"unit": unit_id, "damage": damage})
        return True

    def _block_action(self, unit_id: str, game_state, combo_manager, **kwargs):
        """Defensive block action."""
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_block_action", {"unit": unit_id})
        return True
    ...
    def _counter_attack(self, unit_id: str, game_state, combo_manager, **kwargs):
        ...
        damage = 18
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_counter_attack", {"unit": unit_id, "damage": damage})
        return True

    def _haste_buff(self, unit_id: str, game_state, combo_manager, **kwargs):
        """Speed buff action."""
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_haste_buff", {"unit": unit_id})
        return True

    def _strength_buff(self, unit_id: str, game_state, combo_manager, **kwargs):
        """Strength buff action."""
        if hasattr(combo_manager, "logger") and combo_manager.logger:
            combo_manager.logger.log_event("combo_strength_buff", {"unit": unit_id})
        return True
