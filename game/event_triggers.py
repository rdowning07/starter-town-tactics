    ...
    def execute_effect(self, game_state, event_manager) -> bool:
        ...
        try:
            ...
            if hasattr(event_manager, "logger"):
                event_manager.logger.log_event("event_effect_error", {"event": self.name, "error": str(e)})
            return False
...
class EventManager:
    ...
    def add_event(self, event: EventTrigger) -> bool:
        ...
        if self.logger:
            self.logger.log_event("event_added", {"event": event.name, "description": event.description})
    ...
    def create_event(self, event_name: str, **kwargs) -> Optional[EventTrigger]:
        ...
        if event_name not in self.event_definitions:
            if self.logger:
                self.logger.log_event("event_unknown", {"event": event_name})
            return None
                ...
                if event.execute_effect(game_state, self):
                    ...
                    if self.logger:
                        self.logger.log_event(
                            "event_triggered", {"event": event.name, "description": event.description}
                        )
    ...
    def reset_events(self):
        ...
        if self.logger:
            self.logger.log_event("events_reset", {"count": len(self.events)})
    ...
    def remove_event(self, event_name: str) -> bool:
        ...
        for i, event in enumerate(self.events):
            if event.name == event_name:
                ...
                if self.logger:
                    self.logger.log_event("event_removed", {"event": event_name})
                return True
        ...
        def trap_condition(game_state):
            ...
            for unit_data in game_state.units.units.values():
                if (
                    ...
                    and unit_data.get("x") == position[0]
                    and unit_data.get("y") == position[1]
                ):
                    return True
            return False
    ...
    def _create_death_chain_event(self, chain_length: int = 3, **kwargs) -> EventTrigger:
        ...
        def death_condition(game_state):
            ...
            dead_count = sum(1 for unit_data in game_state.units.units.values() if not unit_data.get("alive", True))
