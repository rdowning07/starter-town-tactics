...
def main() -> None:
    ...
    for step in range(10):
        ...
        if enemy1_unit:
            print(
                f"  E1:   ({enemy1_unit['x']},{enemy1_unit['y']}) HP:{enemy1_unit['hp']} AP:{gs.ap_manager.get_ap('enemy1')}"
            )
        if enemy2_unit:
            print(
                f"  E2:   ({enemy2_unit['x']},{enemy2_unit['y']}) HP:{enemy2_unit['hp']} AP:{gs.ap_manager.get_ap('enemy2')}"
            )
        if enemy3_unit:
            print(
                f"  E3:   ({enemy3_unit['x']},{enemy2_unit['y']}) HP:{enemy3_unit['hp']} AP:{gs.ap_manager.get_ap('enemy3')}"
            )
        ...
        for enemy_id, target_id in enemies:
            if gs.ap_manager.get_ap(enemy_id) > 0:
                ...
                if status == "SUCCESS":
                    ...
                    old_enemy = (
                        enemy1_unit if enemy_id == "enemy1" else enemy2_unit if enemy_id == "enemy2" else enemy3_unit
                    )

                    if (
                        new_enemy
                        and old_enemy
                        and (new_enemy["x"] != old_enemy["x"] or new_enemy["y"] != old_enemy["y"])
                    ):
